...
import { Button, Col, List, Row, Select, Space, Spin, Tooltip, message } from "antd";
import {
    CheckOutlined,
    CloseOutlined,
    EditOutlined,
...
handleClick=()=>{
 ...
 editEvaluation = () =>
 {
  ...
  fetch('/api/evaluations/'+this.props.evaluation.idEvaluation,this.state.optionsU)
  .then(response =>  response.json())  
  .then(response => {console.log(response); console.log(this.state.optionsU);
  ...
  componentDidMount= () => {
    ...
    loadData1 = () =>
    {
      let designations = this.props.evaluation.rubriques.map(elt=>
        elt.designation
      );
      this.setState({dataB:this.props.evaluation.rubriques, data:designations, dataF:this.props.evaluation});
    ...
    loadData2 = () => {
      ...
      .then(response => {
        ...
        this.setState({rubriquesBis:(designations.filter(value => !this.state.data.includes(value))), rubriques:response});
  ...
  editData = (value) => {
    this.setState({rubriquesBis:this.state.rubriquesBis.filter(e=>e!=value)});
  }
  ...
  render() {
   ...
   return (<>
   { this.state.data && this.state.rubriquesBis ?  <><div>
        <h2 style={{ margin: "20px" }}><EditOutlined /> &nbsp; Modifier l'évaluation</h2>
        <ReactDragListView
          ...
          <List
            ...
            renderItem={item => {
              ...
              return (
                ...
                <List.Item onDragStart={() => this.handleDragStart(item)}  onDragEnd={this.handleDragEnd}
                 // actions={["x"]}
                  className={draggble ? "draggble" : ""}
                ><Tooltip placement="topRight" title="Cliquer pour déplacer cette rubrique">
                  
                  <MenuOutlined style={{float:"float", marginRight:"20px", }} /></Tooltip> 
                  <List.Item.Meta title={item} />
                
                <Tooltip placement="topLeft" title="Cliquer pour supprimer cette rubrique de l'evaluation en cours">
                 <Button type="danger" shape="circle" icon={<MinusOutlined />} style={{float:"right", }} size="small" onClick={() => {this.handleDelete(item)}}/> 
                </Tooltip>
      ...
      <Row style={{marginTop:"110px"}}>
        ...
        <Select  style={{ width: 270, }} onChange={this.handleChange} value={this.state.selected} placeholder="Sélectionner une rubrique">
        { 
        this.state.rubriquesBis.map(rubrique=>
          <Option  value={rubrique} key={Math.random()}>{rubrique}</Option>
        )
    } 
        </Select> 
      ...
      <Row style={{marginTop:"90px"}}>
        <Col span={6} offset={5}>
        <Button type="primary" icon={<CheckOutlined />} size="large" onClick={this.handleClick}>
          Valider
        </Button>
        </Col>
        <Col span={6} offset={1}>
        <Button icon={<CloseOutlined />} size="large" onClick={this.handleCancel}>
          Annuler
        </Button>
        </Col>
      </Row>
    ...
    :
    <div style={{textAlign:"center"}}><br /><Spin tip="Loading..." size="large" /></div>
    }
