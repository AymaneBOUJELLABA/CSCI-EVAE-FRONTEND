...
class ShowRes extends React.Component {
  ...
  async fetchRes(){
    ...
    const response = await fetch('http://localhost:8082/api/evaluations/statEval/'+id,
      {
        method:'GET'
      }
    );
  ...
  componentDidMount= () =>
  {
    ...
    ).then(response=>response.json()).then(response=>{console.log("response",response); this.setState({data:response,loading:true})}).catch((err)=>console.log(err));
  ...
  return (
    ...
    <div style = {{height :300 , width: '100%'}}>
        ...
        <h3 style={{ color: "steelblue",fontWeight : "bold"}}>{this.state.data.codeFormation} -{this.state.data.anneeUniversitaire} </h3>
        <Collapse >
        {this.state.loading &&        
        this.state.data.rubriques.map((rubrique) =>
            ...
            <Panel header={rubrique.designation} key={rubrique.idRubrique} style={{fontSize: '15px', fontWeight : "bold"}} >
                 ...
                 <Col span={8}style={{ display:"inline", fontWeight : "bold", textAlign:"center"}}>Question</Col>
                 <Col span={4} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>Minimum</Col>
                 <Col span={1} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>1</Col>
                 <Col span={1} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>2</Col>
                 <Col span={1} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>3</Col>
                 <Col span={1} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>4</Col>
                 <Col span={1} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}>5</Col>
                 <Col span={4} style={{display:"inline", fontWeight : "bold" , textAlign:"center"}}> Maximum</Col> 
                 <Col span={2} style={{display:"inline", fontWeight : "bold", textAlign:"center"}}> Moyenne</Col>                      
                 <Divider />
                </Row>
                {
